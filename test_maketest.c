...
void test_printf(void)
{
	...
	int combocountorig = ft_printf("character[%c]\nstring[%s]\ndecimal[%d]\ninteger[%i]\nlowerHex[%x]\nupperHex[%X]\npointer=[%p]\nunsigned int%u\n", 'a', " string ", INT_MAX, INT_MIN, INT_MAX, INT_MAX, NULL, UINT_MAX);
...
// these will trip the -Wall -Werror -Wextra flag. use make debug
	...
	// int wrong_len = printf("test wrong format [%z]\n", 123); //prints nothing
	// int wrong_len_orig = printf("test wrong format [%z]\n", 123); //prints nothing
